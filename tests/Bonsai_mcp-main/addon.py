...
class BlenderMCPServer:
    ...
    def get_selected_ifc_entities():
        ...
        try:
            ...
            if not selected_objects:
                return {
                    "selected_count": 0,
                    "message": "No objects selected in Blender",
                }
            ...
            for obj in selected_objects:
                if (
                    hasattr(obj, "BIMObjectProperties")
                    and obj.BIMObjectProperties.ifc_definition_id
                ):
    ...
    def list_ifc_entities(entity_type=None, limit=50, selected_only=False):
        ...
        try:
            ...
            if not entity_type:
                ...
                return {
                    "available_types": [
                        {"type": k, "count": v} for k, v in entity_types.items()
                    ]
                }
            ...
            # Prepare the result
            result = {"type": entity_type, "total_count": len(entities), "entities": []}
        ...
        try:
            ...
            for obj in objects:
                ...
                try:
                    ...
                    if containing_structure and containing_structure.is_a(
                        "IfcBuildingStorey"
                    ):
                ...
                obj_data["Description"] = (
                    obj.Description if hasattr(obj, "Description") else None
                )
                obj_data["LevelName"] = container_level

                # Get predefined type if available
                try:
                    obj_data["PredefinedType"] = (
                        ifcopenshell.util.element.get_predefined_type(obj)
                    )
                except:
                ...
                try:
                    ...
                    obj_data["TypeName"] = (
                        type_obj.Name
                        if type_obj and hasattr(type_obj, "Name")
                        else None
                    )
            ...
            for dir_path in output_dirs:
                if (
                    ...
                    and os.access(dir_path, os.W_OK)
                ):
                    output_dir = dir_path
                    break
    ...
    def place_ifc_object(type_name, location, rotation=None):
        ...
        try:
            ...
            if not type_id:
                return {
                    "error": f"Type '{type_name}' not found. Please check if this type exists in the model."
                }
    ...
    def get_ifc_quantities(entity_type=None, selected_only=False):
        ...
        try:
            ...
            sample_elements = (
                file.by_type("IfcElement")[:10] if file.by_type("IfcElement") else []
            )
            ...
            if selected_only:
                ...
                for entity_info in selected_result["selected_entities"]:
                    ...
                    # Extract quantities
                    element_data = extract_quantities(
                        entity, entity_info["blender_name"]
                    )
                    if element_data:
            ...
            # Summary statistics
            summary = {"total_elements": len(elements_data), "element_types": {}}

            # Group by element type for summary
            for element in elements_data:
                etype = element["type"]
                if etype not in summary["element_types"]:
                    summary["element_types"][etype] = {
                        "count": 0,
                        "total_area": 0,
                        "total_volume": 0,
                    }
                ...
                if element["quantities"].get("area"):
                    summary["element_types"][etype]["total_area"] += element[
                        "quantities"
                    ]["area"]
                if element["quantities"].get("volume"):
                    summary["element_types"][etype]["total_volume"] += element[
                        "quantities"
                    ]["volume"]

            return {"success": True, "elements": elements_data, "summary": summary}
    ...
    def export_drawing_png(
        ...
        try:
            ...
            scene.render.engine = (
                "BLENDER_WORKBENCH"  # Fast, good for architectural drawings
            )
            ...
            if view_type == "top":
                ...
                if all_objects:
                    ...
                    building_width = (
                        max(max_x - min_x, max_y - min_y) * 1.2
                    )  # Add 20% margin
                ...
                if all_objects:
                    ...
                    distance = (
                        building_size * 1.2
                    )  # Distance multiplier for good framing
                    ...
                    camera_z = (
                        center_z + distance * 0.3
                    )  # Lower elevation for better facade view
            ...
            if output_path:
                ...
                render_path = os.path.join(
                    temp_dir, f"drawing_{view_type}_{int(time.time())}.png"
                )
            ...
            return {
                "error": f"Error creating drawing: {str(e)}",
        ...
        try:
            ...
            if projects:
                ...
                for ctx in contexts:
                    ...
                    try:
                        if (
                            hasattr(ctx, "HasCoordinateOperation")
                            and ctx.HasCoordinateOperation
                        ):
                            for op in ctx.HasCoordinateOperation:
                                ...
                                if crs:
                                    ...
                                    result["crs"]["geodetic_datum"] = getattr(
                                        crs, "GeodeticDatum", None
                                    )
                                    result["crs"]["vertical_datum"] = getattr(
                                        crs, "VerticalDatum", None
                                    )
                                    try:
                                        map_unit = getattr(crs, "MapUnit", None)
                                        result["crs"]["map_unit"] = (
                                            map_unit.Name if map_unit else None
                                        )
                                    except Exception:
                                    ...
                                    op_entry["target_crs"] = {
                                        ...
                                        "geodetic_datum": result["crs"][
                                            "geodetic_datum"
                                        ],
                                        "vertical_datum": result["crs"][
                                            "vertical_datum"
                                        ],
                                ...
                                if op.is_a("IfcMapConversion"):
                                    mc = {
                                        ...
                                        "orthogonal_height": getattr(
                                            op, "OrthogonalHeight", None
                                        ),
                                        "scale": getattr(op, "Scale", None),
                                        "x_axis_abscissa": getattr(
                                            op, "XAxisAbscissa", None
                                        ),
                                        "x_axis_ordinate": getattr(
                                            op, "XAxisOrdinate", None
                                        ),
                                    }
                ...
                if sites:
                    ...
                    try:
                        if getattr(site, "ObjectPlacement", None):
                            ...
                            axisPlacement = getattr(
                                placement, "RelativePlacement", None
                            )
                            if axisPlacement and getattr(
                                axisPlacement, "Location", None
                            ):
                                loc = axisPlacement.Location
                                if getattr(loc, "Coordinates", None):
                                    result["site"]["local_placement_origin"] = list(
                                        loc.Coordinates
                                    )
                    except Exception as e:
    ...
    def georeference_ifc_model(
        ...
        actions = {
            "created_crs": False,
            "created_map_conversion": False,
            "updated_map_conversion": False,
            "updated_site": False,
            "overwrote": False,
            "wrote_file": False,
        }
        ...
        def select_context():
            ...
            if context_filter:
                for c in ctxs:
                    if (
                        getattr(c, "ContextType", None) or ""
                    ).lower() == context_filter.lower():
                        return c, None
        ...
        if crs_mode == "epsg":
            if not epsg:
                return {
                    "success": False,
                    "error": "epsg code required when crs_mode='epsg'",
                }
            ...
            missing = [
                k
                for k in ("crs_name", "geodetic_datum", "map_projection")
                if locals().get(k) in (None, "")
            ]
            if missing:
                return {
                    "success": False,
                    "error": f"Missing fields for custom CRS: {', '.join(missing)}",
                }
        ...
        try:
            if (
                (eastings is None or northings is None)
                and (
                    site_ref_latitude_dd is not None
                    and site_ref_longitude_dd is not None
                )
                and crs_mode == "epsg"
            ):
                try:
                    ...
                    e, n = transformer.transform(
                        site_ref_longitude_dd, site_ref_latitude_dd
                    )
                    ...
                    warnings.append(
                        f"Could not convert Lat/Long to E/N: {_e}. Provide eastings/northings manually."
                    )
        except Exception as _e:
            warnings.append(
                f"pyproj not available to compute E/N: {_e}. Provide eastings/northings manually."
            )
        ...
        if eastings is None or northings is None:
            return {
                "success": False,
                "error": "eastings and northings are required (or provide lat/long + EPSG with pyproj installed)",
            }
        ...
        if existing_map and not overwrite:
            return {
                ...
                "map_conversion": {
                    ...
                    "orthogonal_height": getattr(
                        existing_map, "OrthogonalHeight", None
                    ),
                ...
                "crs": {
                    "name": getattr(existing_crs, "Name", None)
                    if existing_crs
                    else None,
                    "geodetic_datum": getattr(existing_crs, "GeodeticDatum", None)
                    if existing_crs
                    else None,
                    "map_projection": getattr(existing_crs, "MapProjection", None)
                    if existing_crs
                    else None,
                    "map_zone": getattr(existing_crs, "MapZone", None)
                    if existing_crs
                    else None,
                },
                "warnings": warnings,
        ...
        if existing_crs and overwrite:
            ...
            try:
                ...
                warnings.append(
                    "Could not remove the existing CRS; a new one will be created anyway."
                )
        ...
        if (x_axis_abscissa is None or x_axis_ordinate is None) and (
            true_north_azimuth_deg is not None
        ):
        ...
        orthogonal_height = (
            0.0 if orthogonal_height is None else float(orthogonal_height)
        )
        ...
        if existing_map and overwrite:
            try:
                ...
                warnings.append(
                    "Could not remove the existing MapConversion; another one will be created anyway."
                )
        ...
        try:
            ...
            if sites:
                ...
                warnings.append(
                    "No IfcSite found; lat/long/elevation were not updated."
                )
        except Exception as e:
    ...
    def generate_ids(
        ...
        try:
            ...
            return {
                "ok": False,
                "error": "Could not import ifctester.ids",
                "details": str(e),
            }
        ...
        if not isinstance(specs, list) or len(specs) == 0:
            return {
                "ok": False,
                "error": "You must provide at least one specification in 'specs'.",
            }
        ...
        def _norm_card(c):
            ...
            if c is None:
                return None
            c = str(c).strip().lower()
            if c in ("required", "optional", "prohibited"):
                return c
            raise ValueError(
                "Invalid cardinality: use 'required', 'optional', or 'prohibited'."
            )
        ...
        def _regex_for_threshold(threshold: float, op: str) -> list[str]:
            ...
            if abs(threshold - round(threshold)) < 1e-9:
                ...
                def lt_int(n):
                    if n <= 0:
                        return r"^(?!)$"
                    if n <= 10:
                        return rf"^[0-9]$" if n == 10 else rf"^[0-{n - 1}]$"
                    tens, units = divmod(n - 1, 10)
                    if tens == 1:
                        return r"^([0-9]|1[0-9])$"
                ...
                def le_int(n):
                    ...
                    if tens == 1:
                        return (
                            r"^([0-9]|1[0-9])$"
                            if units == 9
                            else rf"^([0-9]|1[0-{units}])$"
                        )
                    parts = [r"[0-9]"]
                    if tens > 1:
                        parts.append(rf"[1-{tens - 1}]\d")
            ...
            return [
                r"^\d+(?:[.,]\d+)?$"
            ]  # fallback for decimals (plain numeric string)
        ...
        def _build_restriction_for_text(op: str | None, target, bounds: dict):
            ...
            if op and target is not None and _is_number_like(target):
                return ids.Restriction(
                    base="string",
                    options={"pattern": _regex_for_threshold(float(target), op)},
                )
            patterns = []
            ...
            return (
                ids.Restriction(base="string", options={"pattern": patterns})
                if patterns
                else None
            )

        def _build_numeric_restriction(
            dt_upper: str | None, op: str | None, target, bounds: dict
        ):
            """
            Usage:
            ...
            """
            if not (op or any(v is not None for v in bounds.values())):
                return None
        ...
        def _infer_ids_datatype(
            pset: str | None,
            baseName: str | None,
            provided_dt: str | None,
            value,
            op: str | None,
            bounds: dict,
        ) -> str:
            ...
            if provided_dt:
                ...
                if baseName and dtU in ("IFCREAL", "IFCNUMBER", "NUMBER", "REAL"):
                    ...
                    if hint:
                        return hint
                if dtU in MEASURE_HINTS:
                    return MEASURE_HINTS[dtU]
                return dtU
            # hints by name
            if baseName:
                hint = PROPERTY_DATATYPE_HINTS.get(str(baseName).strip().upper())
            ...
            if isinstance(value, ids.Restriction):
                ...
                if base in ("integer",):
                    return "IFCINTEGER"
                if base in ("double", "number", "real", "float"):
                    return "IFCREAL"
                return "IFCLABEL"
        ...
        def _absorb_predefined_type(applicability_list: list):
            ...
            """
            if not isinstance(applicability_list, list):
                return applicability_list
            idx = next(
                (
                    i
                    for i, f in enumerate(applicability_list)
            ...
            for i, f in enumerate(list(applicability_list)):
                if (
                    f.get("type") == "Attribute"
                    and str(f.get("name", "")).strip().upper() == "PREDEFINEDTYPE"
                ):
                    val = f.get("value")
                    if val not in (None, ""):
        ...
        try:
            ...
            try:
                ids_root.title = title or "Untitled"
            except Exception:
                pass
            try:
                ids_root.info.title = title or "Untitled"
            except Exception:
                pass
        except Exception as e:
            return {
                "ok": False,
                "error": "Could not initialize the IDS",
                "details": str(e),
            }
        ...
        def _facet_from_dict(f, spec_desc: str | None, context: str):
            ...
            if t == "Entity":
                ...
                return ids.Entity(
                    name=ent_name,
                    predefinedType=f.get(
                        "predefinedType", ""
                    ),  # we keep it separate (not absorbed)
                ...
                if card:
                    kwargs["cardinality"] = card
                if f.get("cardinality"):
                    kwargs["cardinality"] = _norm_card(f.get("cardinality"))
                ...
                if not pset or not base:
                    raise ValueError("Property requires 'propertySet' and 'baseName'.")
                ...
                if f.get("minValue") is not None:
                    if bool(f.get("minInclusive")):
                        bounds["minInclusive"] = f.get("minValue")
                    else:
                        bounds["minExclusive"] = f.get("minValue")
                if f.get("maxValue") is not None:
                    if bool(f.get("maxInclusive")):
                        bounds["maxInclusive"] = f.get("maxValue")
                    else:
                ...
                if context == "requirements" and (
                    not op
                    and all(v is None for v in bounds.values())
                    and target is None
                    and spec_desc
                ):
                ...
                dt = _infer_ids_datatype(
                    pset, base, f.get("dataType"), val_in, op, bounds
                )
                ...
                if op or any(v is not None for v in bounds.values()):
                    if dt in ("IFCLABEL", "IFCTEXT"):
                        restriction_obj = _build_restriction_for_text(
                            op, target if target is not None else val_in, bounds
                        )
                    else:
                        restriction_obj = _build_numeric_restriction(
                            dt, op, target if target is not None else val_in, bounds
                        )
                ...
                if f.get("cardinality"):
                    kwargs["cardinality"] = _norm_card(f.get("cardinality"))
                if (
                    op or any(v is not None for v in bounds.values())
                ) and "cardinality" not in kwargs:
                ...
                if f.get("uri"):
                    kwargs["uri"] = f["uri"]
                if f.get("cardinality"):
                    kwargs["cardinality"] = _norm_card(f["cardinality"])
                if f.get("instructions"):
                    kwargs["instructions"] = f["instructions"]
        ...
        try:
            for s in specs:
                ...
                if not isinstance(applicability, list) or not isinstance(
                    requirements, list
                ):
                    raise ValueError(
                        "'applicability' and 'requirements' must be lists."
                    )
                ...
                if s.get("name"):
                    try:
                        spec_obj.name = s["name"]
                    except Exception:
                        pass
                if s.get("description"):
                    try:
                        spec_obj.description = s["description"]
                    except Exception:
                        pass
                ...
                try:
                    spec_obj.ifcVersion = canon
                except Exception:
                    pass

                for f in applicability:
                    facet = _facet_from_dict(
                        f, s.get("description"), context="applicability"
                    )
                    spec_obj.applicability.append(facet)
                    total_app += 1

                for f in requirements:
                    facet = _facet_from_dict(
                        f, s.get("description"), context="requirements"
                    )
                    spec_obj.requirements.append(facet)
                    total_req += 1

                ids_root.specifications.append(spec_obj)
                total_specs += 1

        except Exception as e:
            return {
                "ok": False,
                "error": "Error while building the IDS specifications",
                "details": str(e),
            }
        ...
        if total_specs == 0:
            return {
                "ok": False,
                "error": "No Specification was created. Check 'specs'.",
            }
        ...
        try:
            ...
            return {
                "ok": False,
                "error": "Could not save the IDS file",
                "details": str(e),
            }
